...
初版の`max_depth`は、0（無制限）と1（各シードとその直接の内部リンク）だけを
サポートします。既存の非同期キューを使うため、複数ワーカー時は遅いシード1件が
他のワーカーの進行を止めません。比較クロールごとに新しいデータベースを使って
ください。初版の`max_depth: 1`は、データが入ったDBからの再開には対応しません。
...
Ctrl-C（SIGINT/SIGTERM）で安全に停止できます。処理中の状態は永続化され、データベースは正常にクローズされます。同じ`--database`を指定して再実行すれば再開でき、`processing`のまま残った行は次回起動時に自動的に再キューされます。ただし、初版の`max_depth: 1`は新しい空のDBを必要とします。
